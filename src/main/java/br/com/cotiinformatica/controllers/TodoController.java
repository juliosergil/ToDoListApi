...
public class TodoController {
	...
	@DeleteMapping("{id}")
	public ResponseEntity<TodoResponse> delete(@PathVariable("id") Long id, @RequestBody TodoRequest request){
		return ResponseEntity.status(200).body(null);
	}
	...
	@GetMapping("{id}")
	public ResponseEntity<TodoResponse> getById(@PathVariable Long id){ // quando o nome da variável é igual ao parametro recebido, não precisa colocar o ("id") no @PathVariable
		return ResponseEntity.status(200).body(null);
	}
}
